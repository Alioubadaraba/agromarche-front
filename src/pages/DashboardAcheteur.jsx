...
export default function DashboardAcheteur() {
  ...
  const [newOffre, setNewOffre]   = useState({ produit:"Tomates", quantite:"", prix_propose:"", region:"dakar" });
  ...
  useEffect(() => { chargerPrix(); chargerMeteo(); }, [region]);
  ...
  const publierOffre = () => {
    if (!newOffre.quantite || !newOffre.prix_propose) return;
    const offre = { ...newOffre, id: Date.now(), acheteur: user.nom, date: new Date().toLocaleDateString("fr-FR") };
    setOffres([offre, ...offres]);
    setNewOffre({ produit:"Tomates", quantite:"", prix_propose:"", region:"dakar" });
    alert("✅ Offre publiée ! Les agriculteurs de votre région peuvent la voir.");
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="bg-white border-b flex overflow-x-auto">
        {[["marche","📊 Marchés"],["offres","📢 Mes Offres"],["negocier","💰 Négocier"],["meteo","🌦 Météo"]].map(([id,label])=>(
          <button key={id} onClick={()=>setOnglet(id)}
            className={`flex-shrink-0 px-4 py-3 text-sm font-medium border-b-2 transition ${onglet===id?"border-blue-600 text-blue-600":"border-transparent text-gray-500 hover:text-gray-700"}`}>
            {label}
          </button>
        ))}
      </nav>
      ...
      <main className="max-w-2xl mx-auto p-4">
        ...
        {/* MARCHÉS — Prix en temps réel */}
        {onglet==="marche" && (
          <div>
            <h2 className="font-semibold text-gray-700 mb-1">Prix du marché — {region.charAt(0).toUpperCase()+region.slice(1)}</h2>
            <p className="text-xs text-gray-400 mb-4">Comparez les prix avant de négocier avec les agriculteurs</p>
            {prix.length===0 ? (
                ...
                {prix.map((p,i)=>(
                  <div key={i} className="bg-white rounded-xl shadow-sm border p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-gray-800">{p.produit}</p>
                        <p className="text-xs text-gray-400">par {p.unite}</p>
                      </div>
                    </div>
          ...
          <div>
            ...
            <div className="bg-white rounded-xl border p-4 mb-4">
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div>
                  ...
                  <input type="number" placeholder="ex: 500" value={newOffre.quantite}
                    onChange={e=>setNewOffre({...newOffre, quantite:e.target.value})}
                    className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
                </div>
                <div>
              ...
              <button onClick={publierOffre} className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm rounded-lg font-medium transition">
                📢 Publier l'offre
              </button>
            </div>
            ...
            {offres.length === 0 ? (
              <div className="text-center py-8 bg-white rounded-xl border">
                ...
                <p className="text-gray-500 text-sm">Aucune offre publiée pour l'instant</p>
              </div>
            ) : (
              <div className="space-y-3">
                {offres.map((o,i)=>(
                  <div key={i} className="bg-white rounded-xl border p-4">
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center">
                      <div className="bg-gray-50 rounded-lg p-2">
                        ...
                        <p className="font-semibold text-sm">{o.quantite} kg</p>
                      </div>
                      <div className="bg-gray-50 rounded-lg p-2">
                        <p className="text-xs text-gray-400">Prix offert</p>
                        <p className="font-semibold text-sm">{o.prix_propose} FCFA</p>
                      </div>
                      <div className="bg-gray-50 rounded-lg p-2">
                    ...
                    <p className="text-xs text-gray-400 mt-2">Publié le {o.date}</p>
                  </div>
                ))}
              </div>
            )}
        ...
        {onglet==="negocier" && (
          <div>
            ...
            <p className="text-xs text-gray-400 mb-4">Connaissez le prix futur avant de négocier avec les agriculteurs</p>
              ...
              <>
                <div className="grid grid-cols-3 gap-3 mb-4">
                  ...
                  <div className="bg-white rounded-xl border p-3 text-center">
                    ...
                    <p className={`text-lg font-bold ${mlData.variation_pct>=0?"text-red-500":"text-green-600"}`}>{mlData.prix_predit_j30}</p>
                    <p className="text-xs text-gray-400">FCFA/kg</p>
                  </div>
                  <div className="bg-white rounded-xl border p-3 text-center">
                    <p className="text-xs text-gray-400 mb-1">Variation</p>
                    <p className={`text-lg font-bold ${mlData.variation_pct>=0?"text-red-500":"text-green-600"}`}>
                ...
                <div className={`rounded-xl p-4 mb-4 ${mlData.variation_pct>=2?"bg-red-50 border border-red-200":"bg-green-50 border border-green-200"}`}>
                  <p className={`text-sm font-semibold mb-1 ${mlData.variation_pct>=2?"text-red-700":"text-green-700"}`}>
                    💡 Conseil négociation
                  </p>
                  <p className={`text-sm ${mlData.variation_pct>=2?"text-red-600":"text-green-600"}`}>
                    {mlData.variation_pct >= 2
                      ? `⚠️ Prix en hausse prévue. Négociez et achetez maintenant au prix actuel (${mlData.prix_actuel} FCFA) avant que ça monte !`
                      : `✅ Prix en baisse prévue. Attendez quelques jours avant d'acheter — vous économiserez environ ${Math.abs(mlData.prix_actuel - mlData.prix_predit_j30)} FCFA/kg.`
                    }
                  </p>
                </div>
                ...
                {mlData.meilleur_moment && (
                  <div className="bg-white rounded-xl border p-4">
                    <p className="text-sm font-semibold text-gray-700 mb-2">📅 Meilleur moment pour acheter</p>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-green-50 rounded-lg p-3 text-center">
                        <p className="text-xs text-green-600 font-medium">✅ Meilleur prix</p>
                        <p className="text-lg font-bold text-green-700">Jour {mlData.pire_moment?.jour}</p>
                        <p className="text-sm text-green-600">{mlData.pire_moment?.prix_predit} FCFA/kg</p>
                      </div>
                      <div className="bg-red-50 rounded-lg p-3 text-center">
                        <p className="text-xs text-red-500 font-medium">⚠️ Prix le plus haut</p>
                        <p className="text-lg font-bold text-red-600">Jour {mlData.meilleur_moment?.jour}</p>
                        <p className="text-sm text-red-500">{mlData.meilleur_moment?.prix_predit} FCFA/kg</p>
                      </div>
                    </div>
                  </div>
                )}
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[["💧","Humidité",`${meteo.humidite}%`],["🌬","Vent",`${meteo.vent_kmh} km/h`],["🌧","Pluie",`${meteo.pluie_prob}%`]].map(([icon,label,val])=>(
                <div key={label} className="bg-white rounded-xl p-3 text-center shadow-sm border">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
              <p className="text-sm font-semibold text-blue-800 mb-1">💡 Impact sur les prix</p>
              <p className="text-sm text-blue-700">{meteo.conseil}</p>
            </div>
          </div>
        )}
